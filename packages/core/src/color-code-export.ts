...
\`--background\` is the **base canvas** — the empty page with nothing on it. Everything placed on the page (cards, lists, charts, modals) uses \`--card\`, and whatever sits on a card (input fills, lists inside cards, selected rows) uses \`--elevated\`. On mobile, the visible screen is often entirely covered by content, so the entire screen may appear as \`--card\` — but \`--background\` is still the layer underneath, visible in gaps between widgets or behind sheet transitions. On desktop, \`--background\` is more visible because containers don't always fill the viewport. Think of it as: \`--background\` = the wall, \`--card\` = the paper pinned to it, \`--elevated\` = a note stuck on the paper.
