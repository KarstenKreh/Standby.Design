...
export function MotionStage({ model, timeMs, onInterrupt }: MotionStageProps) {
  ...
  const sp = model.spatial ? model.spatial.at(t) : 1;
  ...
  switch (model.demo) {
    ...
    case 'navigate': {
      ...
      content = (
        <div className="absolute overflow-hidden" style={{ inset: PAD }}>
          <Screen label={dir === 1 ? 'List' : 'Detail'} style={{ transform: `translateX(${-dir * sp * inner * 0.3}px)`, opacity: 1 - fxOpacity }} />
          <Screen label={dir === 1 ? 'Detail' : 'List'} style={{ transform: `translateX(${dir * (1 - sp) * inner}px)`, opacity: fxOpacity }} />
        </div>
      );
      break;
    }
