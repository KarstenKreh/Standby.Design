...
ssh "$SERVER" "mkdir -p $REMOTE_DIR/color-react $REMOTE_DIR/type-react $REMOTE_DIR/system-react $REMOTE_DIR/shape-react $REMOTE_DIR/symbol-react $REMOTE_DIR/space-react $REMOTE_DIR/role-react $REMOTE_DIR/motion-react $REMOTE_DIR/qa-gallery $REMOTE_DIR/packages/core $REMOTE_DIR/public"
...
# Upload motion-react source (tar to exclude node_modules/dist, unpack on server)
echo "==> Uploading motion-react source..."
tar cf - --exclude='node_modules' --exclude='dist' --exclude='.git' motion-react/ | ssh "$SERVER" "cd $REMOTE_DIR && rm -rf motion-react && tar xf -"
